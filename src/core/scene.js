...
export class SceneView {
  constructor(host) {
    this.host = host;
    this.scene = new THREE.Scene(); this.scene.background = new THREE.Color(0xe4d9c4);
    ...
    this.hemi = new THREE.HemisphereLight(0xfff6e8, 0x6a5a48, 1.4); this.scene.add(this.hemi);
    const light = new THREE.DirectionalLight(0xfff4e6, 1.6); light.position.set(40, -50, 80); this.scene.add(light);
    this.grid = new THREE.GridHelper(180, 18, 0xb7a88d, 0xd2c6ae); this.scene.add(this.grid);
    ...
    this.guideMaterial = new THREE.MeshStandardMaterial({ color: 0xb4532a, transparent: true, opacity: 0.72, metalness: 0.05, roughness: 0.55, side: THREE.DoubleSide });
    this.guideLineMaterial = new THREE.LineBasicMaterial({ color: 0x6b3418, transparent: true, opacity: 0.9 });
    // The frame is a selection volume only; it is never included in CSG/STL export.
    this.frameMaterial = new THREE.MeshStandardMaterial({ color: 0x4a6740, transparent: true, opacity: 0.28, side: THREE.DoubleSide });
    this.frameLineMaterial = new THREE.LineBasicMaterial({ color: 0x3d5636, transparent: true, opacity: 0.85 });
  ...
  setBaseMesh(mesh) { this.clearPreview(); this.modelGroup.clear(); this.clearGuides(); this.clearFrameGuide(); this.baseMesh = mesh; mesh.material = new THREE.MeshStandardMaterial({ color: 0x9a8b74, metalness: 0.08, roughness: 0.62, side: THREE.DoubleSide }); this.modelGroup.add(mesh); this.focus(mesh); }
